...
public final class NSALoginController {
    ...
    public static void verifyPasswordLengthAndDigit(User user) throws WeakPasswordException {

        String p = user.getPassword();
        int passwordLength = p.length();

        if (passwordLength > 7 && p.matches(".*\\d.*")) {

            System.out.println("Good! Your password has 8 characters at least, and it contains a digit.");
        } else {
            throw new WeakPasswordException("Oh-uh! Your password must be 8 characters long and contain one digit at least.");
        }
